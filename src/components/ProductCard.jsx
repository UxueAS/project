  ...
  return (
    <div key={product.id} className='w-full flex flex-col'>
      ...
      <div className='w-full flex grow items-end'>
        {element === 'productos' ?
          <button onClick={() => addToCart(product, 1)} className={`grow text-white font-light uppercase text-center py-2 mr-1 ${color} hover:bg-primary`}>{btnText}</button>
        : <><button 
            onClick={ token ? ( product.access == 'logged' ?
              () => {setShowSorteoModal(true); } : () => {setShowSubscribeModal(true)})
              : () => navigate('/login')
            }
            className={`grow text-white font-light uppercase text-center py-2 mr-1 ${color} hover:bg-primary transition duration-300`}>{btnText}</button>
